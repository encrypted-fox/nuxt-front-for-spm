<template lang="pug">
  .login
    h1 Вход
    form
      input(placeholder='Логин' type='text' required v-model='login' @keyup.enter='handleLogin' tabindex=1)
      hr
      input(placeholder='Пароль' type='password' required v-model='password' @keyup.enter='handleLogin' tabindex=2)
      svg(width='100'
          height='100'
          viewBox='0 0 100 100'
          fill='none'
...
export default {
  ...
  methods: {
    ...
    async handleLogin() {
      const userLoggedIn = await this.authUser({
        username: this.login,
